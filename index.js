...
function onSubmit() {
  ...
  const mailadress    = document.getElementById('form_answer20')?.value || '';
  const password      = document.getElementById('form_answer21')?.value || '';
  const agreement     = document.getElementById('form_answer02')?.value || '';
  ...
  // パスワード確認
  if (!password) { alert('パスワードを入力してください。'); return false; }
  if (password !== extraField) { alert('パスワードと確認用パスワードが一致しません。'); return false; }

  // 入力チェック＆rawMessage作成
  const fields = [academicType, mailadress, password, agreement, birthDay, birthMonth, birthYear, birthPlace,
                  clubActivity, department, faculty, firstName, firstNameRead, gender, grade, lastName,
                  lastNameRead, phoneNumber, position, universityName];
  let msg = '【送信内容】';
  for (const val of fields) {
    if (!val) { alert('入力項目に漏れがあります。全ての項目への入力をお願い致します。'); return false; }
    msg += '\n' + val;
  }
  ...
  liff.getProfile()
    .then(profile => {
      ...
      const payload = { userId: profile.userId, displayName: profile.displayName, answers };
    ...
    .catch(err => { hideLoading(); alert('送信に失敗しました: ' + err.message); });
