...
function createEventSelectionUI(events) {
    ...
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', () => {
            ...
            // 選択されたイベントリストを更新
            updateSelectedEventsList(selectedEvents);

            // フォームの隠しフィールドを更新
            updateHiddenFields(selectedEvents);
        });
    });
...
/**
 * 選択されたイベントリストを更新する
 * @param {string[]} selectedEvents - 選択されたイベントの配列
 */
function updateSelectedEventsList(selectedEvents) {
    ...
    // ヘッダーを追加
    const header = document.createElement('div');
    header.className = 'selected-events-header';
    header.innerHTML = `
        <h4>選択したイベント (${selectedEvents.length}/2)</h4>
    `;
    ...
    if (selectedEvents.length > 0) {
        ...
        eventsList.className = 'selected-events';
        selectedEvents.forEach(event => {
            const li = document.createElement('li');
            li.textContent = event;
            eventsList.appendChild(li);
        });
        selectedList.appendChild(eventsList);
    } else {
 ...
 * @param {string[]} selectedEvents - 選択されたイベントの配列
 */
function updateHiddenFields(selectedEvents) {
    ...
    if (form_answer22 && form_answer23) {
        form_answer22.value = selectedEvents[0] || '';
        form_answer23.value = selectedEvents[1] || '';
    }
}
